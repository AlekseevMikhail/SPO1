...
struct ast_function_signature_arg ast_function_signature_arg_init(struct ast_type_reference * type, char * id) {
...
struct ast_function_signature_arg_list ast_function_signature_arg_list_init(void) {
    return (struct ast_function_signature_arg_list) {
        .size = 0,
        .capacity = 1,
        .values = malloc(sizeof(struct ast_function_signature_arg)),
    };
}
...
void ast_function_signature_arg_list_append(
    ...
    if (list->size >= list->capacity) {
        ...
        struct ast_function_signature_arg * const new_values =
                realloc(list->values, sizeof(struct ast_function_signature_arg) * new_capacity);
...
struct ast_function_signature * ast_function_signature_new(
    ...
    struct ast_function_signature * result = malloc(sizeof(struct ast_function_signature));
